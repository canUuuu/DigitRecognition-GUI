...
class MNIST(InputPipelineBase):
    ...
    def __init__(self, data_base_dir,
        ...
        print("Training images file:", self._training_images_file)
        print("Training labels file:", self._training_labels_file)
        print("Validation images file:", self._validation_images_file)
        print("Validation labels file:", self._validation_labels_file)
    ...
    def _load_dataset(self, image_file, label_file):
        ...
        try:
            # 直接传文件名字符串给 FixedLengthRecordDataset
            imagedataset = tf.data.FixedLengthRecordDataset(image_file, self.IMG_SIZE * self.IMG_SIZE, header_bytes=16)
            imagedataset = imagedataset.map(self._read_image, num_parallel_calls=self.PARALLEL_INPUT_CALLS)

            labelsdataset = tf.data.FixedLengthRecordDataset(label_file, 1, header_bytes=8)
            labelsdataset = labelsdataset.map(self._read_label, num_parallel_calls=self.PARALLEL_INPUT_CALLS)
